...
VK2DShadowObject
vk2dShadowEnvironmentAddObject(VK2DShadowEnvironment shadowEnvironment) {
  VK2DShadowObject so = VK2D_INVALID_SHADOW_OBJECT;

  // In case the user doesn't use the default one
  if (shadowEnvironment->verticesCount == 0 &&
      shadowEnvironment->objectCount > 0 &&
      shadowEnvironment->objectInfos[0].vertexCount == 0)
    return 0;

  // Reallocate object array
  void *newMem = realloc(shadowEnvironment->objectInfos,
                         sizeof(VK2DShadowObjectInfo) *
                             (shadowEnvironment->objectCount + 1));

  if (newMem != NULL) {
    so = shadowEnvironment->objectCount;
    shadowEnvironment->objectCount++;
    shadowEnvironment->objectInfos = newMem;
    VK2DShadowObjectInfo *soi =
        &shadowEnvironment->objectInfos[shadowEnvironment->objectCount - 1];
    soi->vertexCount = 0;
    soi->startingVertex = shadowEnvironment->verticesCount;
    soi->enabled = true;
    memset(soi->model, 0, sizeof(VK2DMat4));
...
void vk2dShadowEnvironmentObjectSetPos(VK2DShadowEnvironment shadowEnvironment,
                                       VK2DShadowObject object, float x,
                                       float y) {
  VK2DVec3 origin = {x, y, 0};
  memset(shadowEnvironment->objectInfos[object].model, 0, sizeof(VK2DMat4));
...
void vk2dShadowEnvironmentObjectUpdate(VK2DShadowEnvironment shadowEnvironment,
  ...
  if (rotation != 0) {
    ...
    translateMatrix(shadowEnvironment->objectInfos[object].model,
                    originTranslation);
  } else {
    VK2DVec3 origin = {x, y, 0};
    translateMatrix(shadowEnvironment->objectInfos[object].model, origin);
...
void vk2dShadowEnvironmentAddEdge(VK2DShadowEnvironment shadowEnvironment,
  ...
  if (shadowEnvironment->verticesCount == shadowEnvironment->verticesSize) {
    VK2DVec3 *newList = realloc(
        shadowEnvironment->vertices,
  ...
  if (!failedToExtendList) {
    VK2DVec3 vert[6] = {
        {x2, y2, 0}, {x2, y2, 0}, // Projected - 1
        {x1, y1, 0}, {x1, y1, 0}, // Projected - 3
        {x1, y1, 0},              // Projected - 4
        {x2, y2, 0},
    };

    memcpy(&shadowEnvironment->vertices[shadowEnvironment->verticesCount], vert,
           sizeof(VK2DVec3) * 6);
...
void vk2dShadowEnvironmentFlushVBO(VK2DShadowEnvironment shadowEnvironment) {
  ...
  shadowEnvironment->vbo = vk2dBufferLoad(
      ...
      sizeof(VK2DVec3) * shadowEnvironment->verticesCount,
      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, shadowEnvironment->vertices, true);
}