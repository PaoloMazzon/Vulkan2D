...
int main(int argc, const char *argv[]) {
  ...
  // Initialize vk2d
  VK2DRendererConfig config = {.msaa = VK2D_MSAA_32X,
  ...
  VK2DPolygon testPoly = vk2dPolygonShapeCreateRaw((void *)SAMPLE_TRIANGLE,
                                                   SAMPLE_TRIANGLE_VERTICES);
  VK2DPolygon testPoly2 = vk2dPolygonShapeCreateRaw((void *)SAMPLE_RECTANGLE,
  ...
  VK2DCameraSpec cam3D = {VK2D_CAMERA_TYPE_PERSPECTIVE,
                          0,
                          0,
                          WINDOW_WIDTH / 4,
                          WINDOW_HEIGHT / 4,
  ...
  VK2DModel modelVikingRoom =
      vk2dModelLoad("assets/viking_room.obj", texVikingRoom);
  ...
  VK2DShader shader = vk2dSlangLoad("assets/shader.slang");
  handleVK2DStatus();
  ...
  while (!quit && !vk2dStatusFatal()) {
    delta = ((double)SDL_GetPerformanceCounter() - lastTime) /
    ...
    // MSAA controls
    if (keyboard[SDL_SCANCODE_8] || keyboard[SDL_SCANCODE_4] ||
        keyboard[SDL_SCANCODE_2] || keyboard[SDL_SCANCODE_1]) {
    ...
    cam3D.Perspective.fov = (button & SDL_BUTTON_MASK(SDL_BUTTON_RIGHT))
                                ? VK2D_PI * 0.1
                                : VK2D_PI * 0.3;
    ...
    vk2dRendererDrawGeometry((void *)SAMPLE_RECTANGLE,
                             SAMPLE_RECTANGLE_VERTICES, 400, 400, false, 3, 1,
                             1, 0, 0, 0);
    vk2dDrawTexture(testTexture, 0, 0);
    shaderFloat += delta * 5;
    vk2dRendererDrawShader(shader, &shaderFloat, testTexture, 64, 64,
                           4 + 3 * xScale, 4 + 3 * yScale, rot, 8, 8, 0, 0, 16,
                           16);
    shaderFloat += delta * 5;
    vk2dRendererDrawShader(shader, &shaderFloat, testTexture, 250, 170,
                           6 + 3 * xScale, 6 + 3 * yScale,
                           (rot * 0.9) - (VK2D_PI / 2), 8, 8, 0, 0, 16, 16);

    // Draw 3D portions
    vk2dRendererLockCameras(camera3D);
    VK2DVec3 axis = {0, 0, 1};
    vk2dRendererDrawModel(modelVikingRoom, 0, 0, -0.25, 0.75, 0.75, 0.75,
                          sin(-rot / 3) * 0.5, axis, 0, 0, 0);
    vk2dRendererDrawModel(modelCaveguy, 3, 0, -2, 1, 1, 1, -rot / 3, axis, 0, 0,
                          0);
