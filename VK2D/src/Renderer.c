...
static VK2DStartupOptions DEFAULT_STARTUP_OPTIONS = {.enableDebug = false,
                                                     .stdoutLogging = true,
                                                     .quitOnError = true,
                                                     .errorFile =
                                                         "vk2derror.txt",
...
/******************************* User-visible functions (except for being/end
 * kinda) *******************************/

void vk2dRendererStartFrame(const VK2DVec4 clearColour);
  ...
  if (vk2dRendererGetPointer() != NULL) {
    ...
    if (result != VK_SUCCESS) {
      ...
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to get layers, Vulkan error %i.", result);
      return VK2D_ERROR;
    }
    ...
    vkEnumerateInstanceExtensionProperties(
        VK_NULL_HANDLE, &instanceExtensionCount, VK_NULL_HANDLE);
    instanceExtensions =
        malloc(instanceExtensionCount * sizeof(VkExtensionProperties));
    vkEnumerateInstanceExtensionProperties(
        VK_NULL_HANDLE, &instanceExtensionCount, instanceExtensions);
    ...
    sdlExtensions =
        (void *)SDL_Vulkan_GetInstanceExtensions(&sdlExtensionsCount);
    ...
    if (!sdlExtensions) {
      free(gRenderer);
      gRenderer = NULL;
      vk2dRaise(VK2D_STATUS_SDL_ERROR | VK2D_STATUS_VULKAN_ERROR,
                "Failed to get extensions, SDL error %s.", SDL_GetError());
      return VK2D_ERROR;
    }
    ...
    instanceCreateInfo =
        vk2dInitInstanceCreateInfo((void *)&VK2D_DEFAULT_CONFIG, layers,
                                   layerCount, extensions, extensionCount);
    result =
        vkCreateInstance(&instanceCreateInfo, VK_NULL_HANDLE, &gRenderer->vk);

    if (result != VK_SUCCESS) {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to create Vulkan instance, Vulkan error %i.", result);
      free(gRenderer);
      gRenderer = NULL;
      return VK2D_ERROR;
    }
    ...
    gRenderer->ld =
        vk2dLogicalDeviceCreate(gRenderer->pd, false, true,
                                userOptions.enableDebug, &gRenderer->limits);
    if (vk2dStatusFatal()) {
      vk2dRendererQuit();
      return VK2D_ERROR;
    }
    ...
    snprintf((void *)gHostMachineBuffer, gHostMachineBufferSize,
             "%s, SDL %i.%i.%i | Host: %i logical cores, %0.2fgb RAM | Device: "
             "%s, Vulkan %i.%i.%i, Vulkan2D %i.%i.%i",
             SDL_GetPlatform(), SDL_MAJOR_VERSION, SDL_MINOR_VERSION,
             SDL_MICRO_VERSION, SDL_GetNumLogicalCPUCores(),
             (float)SDL_GetSystemRAM() / 1024.0f, props.deviceName,
             VK_VERSION_MAJOR(props.apiVersion),
             VK_VERSION_MINOR(props.apiVersion),
             VK_VERSION_PATCH(props.apiVersion), VK2D_VERSION_MAJOR,
             VK2D_VERSION_MINOR, VK2D_VERSION_PATCH);

    vk2dValidationWriteHeader();

    // Assign user settings, except for screen mode which will be handled later
    gRenderer->config = config;
    gRenderer->config.msaa = gRenderer->limits.maxMSAA >= config.msaa
                                 ? config.msaa
                                 : gRenderer->limits.maxMSAA;
    gRenderer->newConfig = gRenderer->config;

    // Calculate the limit for shader uniform buffers
    gRenderer->limits.maxShaderBufferSize =
        gRenderer->pd->props.limits.maxUniformBufferRange <
                userOptions.vramPageSize
            ? gRenderer->pd->props.limits.maxUniformBufferRange
            : userOptions.vramPageSize;
    gRenderer->limits.maxGeometryVertices =
        (userOptions.vramPageSize / sizeof(VK2DVertexColour)) - 1;
    ...
    allocatorCreateInfo.flags = gRenderer->limits.supportsVRAMUsage
    ...
    // Create budget list
    gRenderer->vmaBudgets =
        malloc(gRenderer->pd->mem.memoryHeapCount * sizeof(VmaBudget));
...
const char *vk2dHostInformation() { return (void *)gHostMachineBuffer; }
...
VK2DRenderer vk2dRendererGetPointer() {
  if (gRenderer == NULL) {
    vk2dRaise(VK2D_STATUS_RENDERER_NOT_INITIALIZED,
...
void vk2dRendererSetConfig(VK2DRendererConfig config) {
  if (vk2dRendererGetPointer() != NULL) {
    ...
    gRenderer->newConfig.msaa = gRenderer->limits.maxMSAA >= config.msaa
                                    ? config.msaa
                                    : gRenderer->limits.maxMSAA;
...
void vk2dRendererGetVRAMUsage(float *inUse, float *total) {
  ...
  *total = 0;
  ...
  for (int i = 0; i < gRenderer->pd->mem.memoryHeapCount; i++) {
    if ((gRenderer->pd->mem.memoryHeaps[i].flags &
  ...
  if (vk2dRendererGetPointer() != NULL) {
    if (!gRenderer->procedStartFrame) {
      ...
      VkResult result = vkAcquireNextImageKHR(
          gRenderer->ld->dev, gRenderer->swapchain, UINT64_MAX,
          gRenderer->imageAvailableSemaphores[gRenderer->currentFrame],
          VK_NULL_HANDLE, &gRenderer->scImageIndex);
      ...
      if (result < 0) {
        if (result == VK_ERROR_DEVICE_LOST) {
          ...
          vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                    "Failed to acquire next image, Vulkan error %i.", result);
        }
        return;
      }

      if (gRenderer->imagesInFlight[gRenderer->scImageIndex] !=
          VK_NULL_HANDLE) {
        vkWaitForFences(gRenderer->ld->dev, 1,
                        &gRenderer->imagesInFlight[gRenderer->scImageIndex],
                        VK_TRUE, UINT64_MAX);
      }
      gRenderer->imagesInFlight[gRenderer->scImageIndex] =
          gRenderer->inFlightFences[gRenderer->currentFrame];
      ...
      gRenderer->targetImage =
          gRenderer->swapchainImages[gRenderer->scImageIndex];
      gRenderer->targetUBOSet =
          gRenderer
              ->uboDescriptorSets[gRenderer->currentFrame]; // TODO: Should prob
                                                            // be reworked
      ...
      result = vkResetCommandBuffer(
          gRenderer->commandBuffer[gRenderer->scImageIndex], 0);
      VkResult result2 = vkResetCommandBuffer(
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex], 0);
      VkResult result3 = vkResetCommandBuffer(
          gRenderer->computeCommandBuffer[gRenderer->scImageIndex], 0);
      if (result != VK_SUCCESS || result2 != VK_SUCCESS ||
          result3 != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_OUT_OF_VRAM,
                  "Failed to reset command buffer at start of frame.");
        return;
      }
      result = vkBeginCommandBuffer(
          gRenderer->commandBuffer[gRenderer->scImageIndex], &beginInfo);
      result2 = vkBeginCommandBuffer(
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex], &beginInfo);
      result3 = vkBeginCommandBuffer(
          gRenderer->computeCommandBuffer[gRenderer->scImageIndex], &beginInfo);
      if (result != VK_SUCCESS || result2 != VK_SUCCESS ||
          result3 != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to begin command buffer at start of frame, Vulkan "
                  "error %i/%i/%i.",
      ...
      vk2dDescriptorBufferBeginFrame(
          gRenderer->descriptorBuffers[gRenderer->currentFrame],
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex]);
      ...
      VkRenderPassBeginInfo renderPassBeginInfo = vk2dInitRenderPassBeginInfo(
          ...
          gRenderer->framebuffers[gRenderer->scImageIndex], rect, clearValues,
          clearCount);

      vkCmdBeginRenderPass(gRenderer->commandBuffer[gRenderer->scImageIndex],
                           &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

      // Bind compute pipeline to the compute buffer
      vkCmdBindPipeline(
          gRenderer->computeCommandBuffer[gRenderer->scImageIndex],
          VK_PIPELINE_BIND_POINT_COMPUTE,
          vk2dPipelineGetCompute(gRenderer->spriteBatchPipe));
    }
  }
}
    ...
    if (gRenderer->procedStartFrame) {
      ...
      for (int i = 0; i < VK2D_MAX_CAMERAS; i++)
        if (gRenderer->cameras[i].state == VK2D_CAMERA_STATE_NORMAL)
          _vk2dCameraUpdateUBO(&gRenderer->workingUBO,
                               &gRenderer->cameras[i].spec, i);
      ...
      vk2dDescriptorBufferEndFrame(
          gRenderer->descriptorBuffers[gRenderer->currentFrame],
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex]);

      // Record necessary pipeline barriers to the copy and compute buffers
      vk2dDescriptorBufferRecordCopyPipelineBarrier(
          gRenderer->descriptorBuffers[gRenderer->currentFrame],
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex]);
      vk2dDescriptorBufferRecordComputePipelineBarrier(
          gRenderer->descriptorBuffers[gRenderer->currentFrame],
          gRenderer->computeCommandBuffer[gRenderer->scImageIndex]);

      // PRESENT
      VkResult result =
          vkEndCommandBuffer(gRenderer->commandBuffer[gRenderer->scImageIndex]);
      VkResult result2 = vkEndCommandBuffer(
          gRenderer->dbCommandBuffer[gRenderer->scImageIndex]);
      VkResult result3 = vkEndCommandBuffer(
          gRenderer->computeCommandBuffer[gRenderer->scImageIndex]);
      if (result != VK_SUCCESS || result2 != VK_SUCCESS ||
          result3 != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
      ...
      VkSubmitInfo submitInfo = vk2dInitSubmitInfo(
          bufs, 3,
          &gRenderer->renderFinishedSemaphores[gRenderer->scImageIndex], 1,
          &gRenderer->imageAvailableSemaphores[gRenderer->currentFrame], 1,
          waitStage);

      // Submit queue
      if (vkResetFences(gRenderer->ld->dev, 1,
                        &gRenderer->inFlightFences[gRenderer->currentFrame]) !=
          VK_SUCCESS) {
      ...
      result =
          vkQueueSubmit(gRenderer->ld->queue, 1, &submitInfo,
                        gRenderer->inFlightFences[gRenderer->currentFrame]);
      ...
      if (result < 0) {
        if (result == VK_ERROR_DEVICE_LOST)
          ...
          vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                    "Failed to submit queue, Vulkan error %i.", result);
        return VK2D_ERROR;
      }

      // Synchronization depends on if nuklear is enabled
      VkSemaphore presentWaitSemaphores[1] = {
      ...
      VkPresentInfoKHR presentInfo = vk2dInitPresentInfoKHR(
          &gRenderer->swapchain, 1, &gRenderer->scImageIndex, &result,
          presentWaitSemaphores, 1);
      VkResult queueRes = vkQueuePresentKHR(gRenderer->ld->queue, &presentInfo);
      if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
          gRenderer->resetSwapchain || queueRes == VK_ERROR_OUT_OF_DATE_KHR) {
        _vk2dRendererResetSwapchain();
        gRenderer->resetSwapchain = false;
        res = VK2D_RESET_SWAPCHAIN;
      } else if (result < 0 || queueRes < 0) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to present frame, Vulkan error %i/%i.", queueRes,
                  result);
      }

      gRenderer->currentFrame =
          (gRenderer->currentFrame + 1) % VK2D_MAX_FRAMES_IN_FLIGHT;

      // Calculate time
      gRenderer->accumulatedTime +=
          (((double)SDL_GetPerformanceCounter() - gRenderer->previousTime) /
           (double)SDL_GetPerformanceFrequency()) *
          1000;
      gRenderer->amountOfFrames++;
      if (gRenderer->accumulatedTime >= 1000) {
        gRenderer->frameTimeAverage =
            gRenderer->accumulatedTime / gRenderer->amountOfFrames;
...
void vk2dRendererSetTarget(VK2DTexture target) {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    if (target != gRenderer->target) {
      ...
      VkRenderPass pass = target == VK2D_TARGET_SCREEN
                              ? gRenderer->midFrameSwapRenderPass
                              : gRenderer->externalTargetRenderPass;
      VkFramebuffer framebuffer =
          target == VK2D_TARGET_SCREEN
              ? gRenderer->framebuffers[gRenderer->scImageIndex]
              : target->fbo;
      VkImage image = target == VK2D_TARGET_SCREEN
                          ? gRenderer->swapchainImages[gRenderer->scImageIndex]
                          : target->img->img;
      VkDescriptorSet buffer =
          target == VK2D_TARGET_SCREEN
              ? gRenderer->uboDescriptorSets[gRenderer->currentFrame]
              : target->uboSet;

      vkCmdEndRenderPass(gRenderer->commandBuffer[gRenderer->scImageIndex]);

      // Now we either have to transition the image layout depending on whats
      // going in and whats poppin out
      if (target == VK2D_TARGET_SCREEN)
        _vk2dTransitionImageLayout(gRenderer->targetImage,
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
      else
        _vk2dTransitionImageLayout(target->img->img,
                                   VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
      ...
      VkRect2D rect = {0};
      rect.extent.width = target == VK2D_TARGET_SCREEN ? gRenderer->surfaceWidth
                                                       : target->img->width;
      rect.extent.height = target == VK2D_TARGET_SCREEN
                               ? gRenderer->surfaceHeight
                               : target->img->height;
      VkClearValue clear[2] = {0};
      clear[1].depthStencil.depth = 1;
      VkRenderPassBeginInfo renderPassBeginInfo =
          vk2dInitRenderPassBeginInfo(pass, framebuffer, rect, clear, 2);
...
void vk2dRendererClear() {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    ...
    _vk2dRendererDrawRaw(&set, 1, gRenderer->unitSquare,
                         gRenderer->primFillPipe, 0, 0, 1, 1, 0, 0, 0, 1, 0, 0,
                         0, 0, VK2D_INVALID_CAMERA);
  }
}
...
void vk2dRendererEmpty() {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    ...
    const VK2DVec4 clearColour = {0, 0, 0, 0};
...
void vk2dRendererDrawRectangle(float x, float y, float w, float h, float r,
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    vk2dRendererFlushSpriteBatch();
    vk2dRendererDrawPolygon(gRenderer->unitSquare, x, y, true, 1, w, h, r,
                            ox / (w / 3), oy / (h / 3));
  }
}
...
void vk2dRendererDrawRectangleOutline(float x, float y, float w, float h,
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    vk2dRendererFlushSpriteBatch();
    vk2dRendererDrawPolygon(gRenderer->unitSquareOutline, x, y, false,
                            lineWidth, w, h, r, ox / (w / 3), oy / (h / 3));
  }
}
...
void vk2dRendererDrawCircle(float x, float y, float r) {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    vk2dRendererFlushSpriteBatch();
    vk2dRendererDrawPolygon(gRenderer->unitCircle, x, y, true, 1, r * 2, r * 2,
                            0, 0, 0);
  }
}
...
void vk2dRendererDrawCircleOutline(float x, float y, float r, float lineWidth) {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    vk2dRendererFlushSpriteBatch();
    vk2dRendererDrawPolygon(gRenderer->unitCircleOutline, x, y, false,
                            lineWidth, r * 2, r * 2, 0, 0, 0);
  }
}
...
void vk2dRendererDrawLine(float x1, float y1, float x2, float y2) {
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    ...
    vk2dRendererDrawPolygon(gRenderer->unitLine, x1, y1, false, 1, x, 1, r, 0,
                            0);
  }
}
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    if (shader != NULL) {
      ...
      if (shader->uniformSize != 0) {
        sets[3] = vk2dDescConGetSet(
            gRenderer->descConShaders[gRenderer->currentFrame]);
        VkBuffer buffer;
        VkDeviceSize offset;
        vk2dDescriptorBufferCopyData(
            gRenderer->descriptorBuffers[gRenderer->currentFrame], data,
            shader->uniformSize, &buffer, &offset);
        ...
        vkUpdateDescriptorSets(gRenderer->ld->dev, 1, &write, 0,
                               VK_NULL_HANDLE);
        setCount = 4;
      }

      _vk2dRendererDrawShader(sets, setCount, tex, shader->pipe, x, y, xscale,
                              yscale, rot, originX, originY, 1, xInTex, yInTex,
                              texWidth, texHeight);
    } else {
...
void vk2dRendererDrawPolygon(VK2DPolygon polygon, float x, float y, bool filled,
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    ...
    if (polygon != NULL) {
      ...
      _vk2dRendererDraw(
          &set, 1, polygon,
          filled ? gRenderer->primFillPipe : gRenderer->primLinePipe, x, y,
          xscale, yscale, rot, originX, originY, lineWidth, 0, 0, 0, 0);
    } else {
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    if (vertices != NULL && count > 0) {
      ...
      if (count <= gRenderer->limits.maxGeometryVertices) {
        ...
        VkDeviceSize offset;
        vk2dDescriptorBufferCopyData(
            gRenderer->descriptorBuffers[gRenderer->currentFrame], vertices,
...
void vk2dRendererDrawModel(VK2DModel model, float x, float y, float z,
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    if (model != NULL) {
      ...
      sets[2] = gRenderer->texArrayDescriptorSet;
      _vk2dRendererDraw3D(sets, 3, model, gRenderer->modelPipe, x, y, z, xscale,
                          yscale, zscale, rot, axis, originX, originY, originZ,
                          1);
    } else {
...
void vk2dRendererDrawWireframe(VK2DModel model, float x, float y, float z,
  ...
  if (vk2dRendererGetPointer() != NULL && !vk2dStatusFatal()) {
    if (model != NULL) {
      ...
      _vk2dRendererDraw3D(sets, 3, model, gRenderer->wireframePipe, x, y, z,
                          xscale, yscale, zscale, rot, axis, originX, originY,
                          originZ, lineWidth);
    } else {
      vk2dRaise(VK2D_STATUS_BAD_ASSET, "Model does not exist.");
    }
...
static void _vk2dRendererFlushPerCamera(VkCommandBuffer buf, int cameraIndex) {
  ...
  VK2DInstancedPushBuffer push = {.cameraIndex = cameraIndex};
  VkRect2D scissor;
  VkViewport viewport;
  ...
  vkCmdPushConstants(buf, gRenderer->currentBatchPipeline->layout,
                     VK_SHADER_STAGE_VERTEX_BIT, 0,
                     sizeof(struct VK2DInstancedPushBuffer), &push);
...
void vk2dRendererFlushSpriteBatch() {
  ...
  //  4. Send out the draw command that uses the soon-to-be-filled compute
  //  output as vertex input
  if (gRenderer->currentBatchPipeline != NULL &&
    ...
    vk2dDescriptorBufferCopyData(
        ...
        &drawCommands, &drawCommandsOffset);

    // Reserve space for the draw instances
    vk2dDescriptorBufferReserveSpace(
        gRenderer->descriptorBuffers[gRenderer->currentFrame],
        gRenderer->drawCommandCount * sizeof(VK2DDrawInstance), &drawInstances,
        &drawInstancesOffset);

    // Create descriptor sets
    const uint32_t drawCount = gRenderer->drawCommandCount;
    VkDescriptorSet descriptorSet =
        vk2dDescConGetSet(gRenderer->descConCompute[gRenderer->currentFrame]);
    VkDescriptorSet vertexShaderSBOSet =
        vk2dDescConGetSet(gRenderer->descConSBO[gRenderer->currentFrame]);
    ...
    // Queue compute dispatches to the compute command buffer, synchronization
    // will be recorded at the end of the frame
    VkCommandBuffer computeBuf =
        gRenderer->computeCommandBuffer[gRenderer->scImageIndex];
    VK2DComputePushBuffer push = {.drawCount = drawCount};
    vkCmdPushConstants(computeBuf, gRenderer->spriteBatchPipe->layout,
                       VK_SHADER_STAGE_COMPUTE_BIT, 0,
                       sizeof(VK2DComputePushBuffer), &push);
    vkCmdBindDescriptorSets(computeBuf, VK_PIPELINE_BIND_POINT_COMPUTE,
                            gRenderer->spriteBatchPipe->layout, 0, 1,
    ...
    VkDescriptorSet sets[] = {
        gRenderer->target != NULL && !gRenderer->enableTextureCameraUBO
            ? gRenderer->targetUBOSet
            : gRenderer->uboDescriptorSets[gRenderer->currentFrame],
        gRenderer->samplerSet, gRenderer->texArrayDescriptorSet,
        vertexShaderSBOSet};
    // These things are the same across every camera, so they are only bound
    // once
    vkCmdBindDescriptorSets(buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            gRenderer->instancedPipe->layout, 0, 4, sets, 0,
                            VK_NULL_HANDLE);
