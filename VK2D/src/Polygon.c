...
VK2DPolygon _vk2dPolygonCreate(VK2DLogicalDevice dev, void *data, uint32_t size,
                               VK2DVertexType type) {
  VK2DPolygon poly = malloc(sizeof(struct VK2DPolygon_t));

  if (poly != NULL) {
    VK2DBuffer buf = vk2dBufferLoad(
        dev, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, data, true);
    if (buf != NULL) {
      poly->vertices = buf;
      poly->type = type;
    } else {
...
VK2DPolygon vk2dPolygonShapeCreateRaw(VK2DVertexColour *vertexData,
  ...
  VK2DPolygon poly = _vk2dPolygonCreate(dev, vertexData,
                                        sizeof(VK2DVertexColour) * vertexCount,
...
VK2DPolygon vk2dPolygonCreate(const VK2DVec2 *vertices, uint32_t vertexCount) {
  ...
  VK2DVertexColour *colourVertices =
      malloc(sizeof(VK2DVertexColour) * finalVertexCount);
  uint32_t i;
  ...
  if (colourVertices != NULL) {
    ...
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate triangulation buffer for %i vertices.",
              vertexCount);
  }
...
VK2DPolygon vk2dPolygonCreateOutline(const VK2DVec2 *vertices,
  ...
  VK2DPolygon out = NULL;
  VK2DVertexColour *colourVertices =
      malloc(sizeof(VK2DVertexColour) * (vertexCount + 1));
