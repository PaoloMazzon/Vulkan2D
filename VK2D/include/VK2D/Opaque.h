...
/// \brief An internal representation of a camera (the user deals with
/// VK2DCameraIndex, the renderer uses this struct)
///
...
typedef struct _VK2DDescriptorBufferInternal {
  VK2DBuffer deviceBuffer; ///< Device-local (on vram) buffer that the shaders
                           ///< will access
  VK2DBuffer
      stageBuffer; ///< Host-local (on ram) buffer that data will be copied into
...
///
/// It will put a event barrier into the startframe command buffer where drawing
/// happens, and then it will put a memory copy into the second command buffer
/// at the end of the frame so Vulkan copies the buffer to device memory all at
/// once instead of tiny increments.
struct VK2DDescriptorBuffer_t {
  _VK2DDescriptorBufferInternal *buffers; ///< List of internal buffers so that
                                          ///< we can allocate more on the fly
  int bufferCount; ///< Amount of internal buffers in the descriptor buffer, for
  ...
  VkBufferMemoryBarrier *memoryBarriers; ///< List of barriers that matches the
                                         ///< size of the buffer list size
};

/// \brief Abstraction for descriptor pools and sets so you can dynamically use
/// them
struct VK2DDescCon_t {
  ...
  // pools will always have poolListSize elements, but only elements up to
  // poolsInUse will be valid pools (in an effort to avoid constantly
  // reallocating memory)
  uint32_t poolsInUse;   ///< Number of actively in use pools in pools
  uint32_t poolListSize; ///< Total length of pools array
};

/// \brief A handy abstraction that groups up pipeline state and makes multiple
/// shaders easier
struct VK2DPipeline_t {
  ...
  VkClearValue
      clearValue[2]; ///< Clear values for the two attachments: colour and depth
  int32_t id;        ///< Unique id for this pipeline
  VkPipeline
      pipes[VK2D_BLEND_MODE_MAX]; ///< Internal pipelines if
                                  ///< `VK2D_GENERATE_BLEND_MODES` is enabled
};
...
///
/// There are some limitations of shaders and some things to be aware of. For
/// one, you can only have one uniform buffer that is sent to both shaders.
/// Also, for each shader, the renderer must go through and increment the
/// current uniform buffer every frame to make sure you don't modify one that is
/// currently in an executing command buffer.
///
/// And most importantly, these shaders are all treated as texture shaders,
/// which is to say they will all receive the same data that the tex.vert and
/// tex.frag shaders would (push constants, vertex attributes, and uniforms) in
/// addition to a user-defined uniform buffer if one is specified.
struct VK2DShader_t {
...
///
/// Only textures created with vk2dTextureCreate may be rendered to. Should you
/// try to set the render target to a texture not created with
/// vk2dTextureCreate, you can expect a segfault.
struct VK2DTexture_t {
  ...
  bool imgHandled; ///< Whether or not to free the image with the texture (if it
                   ///< was loaded with vk2dTextureLoad)
  SDL_AtomicInt
      descriptorIndex; ///< Index in the descriptor array of this texture
};
...
struct VK2DShadowEnvironment_t {
  ...
  VK2DVec3 *vertices; ///< Raw vertices before they get shipped off to the gpu
...
struct VK2DGui_t {
  ...
  bool fontsLoaded; // ok if no fonts loaded, but we need to make the font atlas
                    // regardless
};

/// \brief Core rendering data, don't modify values unless you know what you're
/// doing
struct VK2DRenderer_t {
  // Devices/core functionality (these have short names because they're
  ...
  VmaBudget *vmaBudgets; ///< List of blank VMA budgets to be used later, list
                         ///< size if number of heaps on device

  // User-end things
  VK2DRendererConfig config;    ///< User config
  VK2DRendererConfig newConfig; ///< In the event that its updated, we only swap
                                ///< out when we're ready to reset the swapchain
  bool resetSwapchain; ///< If true, the swapchain (effectively the whole thing)
                       ///< will reset on the next rendered frame
  VK2DImage msaaImage; ///< In case MSAA is enabled
  VK2DVec4 colourBlend; ///< Used to modify colours (and transparency) of
                        ///< anything drawn. Passed via push constants.
  VkSampler textureSampler; ///< Needed for textures
  VkSampler modelSampler;   ///< Same as textureSampler but for 3D (normalized
                            ///< coordinates)
  VkViewport viewport;      ///< Viewport to draw with
  bool enableTextureCameraUBO; ///< If true, when drawing to a texture the UBO
                               ///< for the internal camera is used instead of
                               ///< the texture's UBO
  ...
  VK2DCameraIndex
      cameraLocked; ///< If true, only the default camera will be drawn to
  VK2DStartupOptions options; ///< Root options for the renderer
  VK2DRendererLimits limits;  ///< For user safety

  // Cameras/ubos
  VkDescriptorSet
      uboDescriptorSets[VK2D_MAX_FRAMES_IN_FLIGHT]; ///< Descriptor sets holding
                                                    ///< a UBO per frame in
                                                    ///< flight
  VK2DUniformBufferObject workingUBO; ///< This frame's ubo, basically just has
                                      ///< VK2D_MAX_CAMERAS viewproj matricies
  ...
  // Swapchain
  VkSwapchainKHR
      swapchain; ///< Swapchain (manages images and presenting to screen)
  ...
  VkRenderPass midFrameSwapRenderPass; ///< Render pass for mid-frame switching
                                       ///< back to the swapchain as a target
  VkRenderPass
      externalTargetRenderPass; ///< Render pass for rendering to textures
  ...
  bool procedStartFrame; ///< End frame things are only done if this is true and
                         ///< start frame things are only done if this is false
  ...
  VkDescriptorSetLayout
      dslSampler; ///< Descriptor set layout for texture samplers
  VkDescriptorSetLayout
      dslBufferVP; ///< Descriptor set layout for the view-projection buffer
  VkDescriptorSetLayout dslBufferUser; ///< Descriptor set layout for user data
                                       ///< buffers (custom shaders uniforms)
  VkDescriptorSetLayout dslTexture; ///< Descriptor set layout for the textures
  VkDescriptorSetLayout
      dslTextureArray; ///< Descriptor set layout for texture array
  VkDescriptorSetLayout
      dslSpriteBatch;                 ///< DSL for compute sprite batch pipeline
  ...
  VK2DDescCon
      descConSamplersOff; ///< Descriptor controller for samplers off thread
  ...
  VK2DDescCon
      descConCompute[VK2D_MAX_FRAMES_IN_FLIGHT]; ///< Descriptor controller for
                                                 ///< sprite batch compute
                                                 ///< buffer
  VK2DDescCon
      descConShaders[VK2D_MAX_FRAMES_IN_FLIGHT]; ///< Descriptor controller for
                                                 ///< user shaders
  VK2DDescCon
      descConSBO[VK2D_MAX_FRAMES_IN_FLIGHT]; ///< Descriptor controller for
                                             ///< sprite batch vertex shader SBO
  ...
  VK2DDescriptorBuffer
      *descriptorBuffers;       ///< Descriptor buffer, one per frame in flight
  ...
  VkSemaphore *imageAvailableSemaphores; ///< Semaphores to signal when the
                                         ///< image is ready
  VkSemaphore *
      renderFinishedSemaphores; ///< Semaphores to signal when rendering is done
  ...
  VK2DTexture
      *targets; ///< List of all currently loaded textures targets (in case the
                ///< MSAA is changed and the sample image needs to be reloaded)
  uint32_t targetListSize; ///< Amount of elements in the list (only non-null
                           ///< elements count)

  // Optimization tools - if the renderer knows the proper set/pipeline/vbo is
  // already bound it doesn't need to rebind it
  ...
  double
      amountOfFrames; ///< Number of frames needed to calculate frameTimeAverage
  double accumulatedTime;  ///< Total time of frames for average in ms
  double frameTimeAverage; ///< Average amount of time frames are taking over a
                           ///< second (in ms)
