...
const VK2DVec2 POLY_1[] = {
    {13, 74}, {99, 35}, {152, 61}, {151, 108}, {74, 122},
};
#define POLY_1_COUNT (sizeof(POLY_1) / sizeof(VK2DVec2))

const VK2DVec2 POLY_2[] = {
    {216, 79}, {230, 45}, {331, 52}, {365, 127}, {363, 176}, {293, 133},
};
#define POLY_2_COUNT (sizeof(POLY_2) / sizeof(VK2DVec2))

const VK2DVec2 POLY_3[] = {
    {340, 228},
    {361, 250},
    {332, 248},
};
#define POLY_3_COUNT (sizeof(POLY_3) / sizeof(VK2DVec2))

const VK2DVec2 POLY_4[] = {
    {232, 178}, {285, 223}, {275, 275}, {140, 230}, {139, 226},
};
#define POLY_4_COUNT (sizeof(POLY_4) / sizeof(VK2DVec2))

const VK2DVec2 POLY_5[] = {
    {42, 165},
    {88, 178},
    {93, 220},
    {22, 218},
};
#define POLY_5_COUNT (sizeof(POLY_5) / sizeof(VK2DVec2))

const VK2DVec2 MOVING_POLY[] = {
    {-10, 10},
    {10, 10},
    {10, -10},
    {-10, -10},
};
#define MOVING_POLY_COUNT (sizeof(MOVING_POLY) / sizeof(VK2DVec2))

const VK2DVec2 *POLYGONS[] = {POLY_1, POLY_2, POLY_3,
                              POLY_4, POLY_5, MOVING_POLY};
const int POLYGON_COUNTS[] = {POLY_1_COUNT, POLY_2_COUNT, POLY_3_COUNT,
                              POLY_4_COUNT, POLY_5_COUNT, MOVING_POLY_COUNT};
...
int main(int argc, const char *argv[]) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("VK2D", WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_VULKAN);
  SDL_Event e;
  bool quit = false;
  int keyboardSize;
  ...
  for (int i = 0; i < POLYGON_COUNT; i++) {
    ...
    polygonOutlines[i] =
        vk2dPolygonCreateOutline(POLYGONS[i], POLYGON_COUNTS[i]);
  }
  ...
  for (int i = 0; i < POLYGON_COUNT; i++) {
    ...
    for (int vertex = 0; vertex < POLYGON_COUNTS[i]; vertex++) {
      ...
      vk2dShadowEnvironmentAddEdge(
          shadows, POLYGONS[i][prevIndex][0], POLYGONS[i][prevIndex][1],
          POLYGONS[i][vertex][0], POLYGONS[i][vertex][1]);
    }
  }
  vk2dShadowEnvironmentFlushVBO(shadows);

  // Camera
  VK2DCameraSpec cam = {VK2D_CAMERA_TYPE_DEFAULT, 0, 0, WINDOW_WIDTH * 0.5f,
                        WINDOW_HEIGHT * 0.5f,     1, 0};
  ...
  while (!quit) {
    ...
    // Move player
    double moveX = (double)keyboard[SDL_SCANCODE_RIGHT] -
                   (double)keyboard[SDL_SCANCODE_LEFT];
    double moveY =
        (double)keyboard[SDL_SCANCODE_DOWN] - (double)keyboard[SDL_SCANCODE_UP];
    velocityX +=
        moveX != 0 ? moveX * ACCELERATION : towardsZero(velocityX, FRICTION);
    velocityY +=
        moveY != 0 ? moveY * ACCELERATION : towardsZero(velocityY, FRICTION);
    velocityX = clamp(velocityX, -MAX_VELOCITY, MAX_VELOCITY);
    velocityY = clamp(velocityY, -MAX_VELOCITY, MAX_VELOCITY);
    playerX += velocityX;
    playerY += velocityY;
    playerX = playerX > windowWidth * 0.5
                  ? -vk2dTextureWidth(playerTex)
                  : (playerX < -vk2dTextureWidth(playerTex) ? windowWidth * 0.5
                                                            : playerX);
    playerY =
        playerY > windowHeight * 0.5
            ? -vk2dTextureHeight(playerTex)
            : (playerY < -vk2dTextureHeight(playerTex) ? windowHeight * 0.5
                                                       : playerY);
    mousePolyRot += VK2D_PI * 0.01;

    // Lock framerate to 60
    vk2dSleep((((double)SDL_GetPerformanceFrequency() / 60) -
               ((double)SDL_GetPerformanceCounter() - lastTime)) /
              SDL_GetPerformanceFrequency());
    lastTime = SDL_GetPerformanceCounter();

    // Draw the lights
    vk2dRendererLockCameras(testCamera);
    vk2dShadowEnvironmentObjectUpdate(shadows, mouseShadowObject, mouseX,
                                      mouseY, 1, 1, mousePolyRot, 0, 0);
    ...
    for (int i = 0; i < lightCount; i++) {
      ...
      vk2dDrawTexture(lightOrbTex,
                      lights[i].pos[0] - (vk2dTextureWidth(lightOrbTex) / 2),
                      lights[i].pos[1] - (vk2dTextureHeight(lightOrbTex) / 2));
    ...
    for (int i = 0; i < POLYGON_COUNT; i++) {
      if (i != MOUSE_POLYGON_INDEX) {
        ...
        vk2dRendererDrawPolygon(polygonOutlines[i], 0, 0, false, 3, 1, 1, 0, 0,
                                0);
        vk2dRendererSetColourMod(VK2D_DEFAULT_COLOUR_MOD);
      } else {
        vk2dRendererDrawPolygon(polygons[i], mouseX, mouseY, true, 0, 1, 1,
                                mousePolyRot, 0, 0);
        vk2dRendererSetColourMod(VK2D_BLACK);
        vk2dRendererDrawPolygon(polygonOutlines[i], mouseX, mouseY, false, 3, 1,
                                1, mousePolyRot, 0, 0);
        vk2dRendererSetColourMod(VK2D_DEFAULT_COLOUR_MOD);
      }
    }
    vk2dDrawTexture(playerTex, playerX - (vk2dTextureWidth(playerTex) / 2),
                    playerY - (vk2dTextureHeight(playerTex) / 2));
