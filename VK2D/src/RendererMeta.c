...
VK2DVec2 unitSquareOutline[] = {{0, 0}, {1, 0}, {1, 1}, {0, 1}};
uint32_t unitSquareOutlineVertices = 4;

const VK2DVec2 LINE_VERTICES[] = {{0, 0}, {1, 0}};
const uint32_t LINE_VERTEX_COUNT = 2;

// Renderer has to keep track of user shaders in case the swapchain gets
// recreated
VK2DResult _vk2dRendererAddShader(VK2DShader shader) {
  ...
  if (found == UINT32_MAX) {
    VK2DShader *newList =
        realloc(gRenderer->customShaders,
                sizeof(VK2DShader) *
    ...
    if (newList != NULL) {
      ...
      for (i = gRenderer->shaderListSize;
           i < gRenderer->shaderListSize + VK2D_DEFAULT_ARRAY_EXTENSION; i++) {
      ...
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to extend shaders list for %i shaders.",
                gRenderer->shaderListSize);
      return VK2D_ERROR;
    }
  }
...
void _vk2dRendererAddTarget(VK2DTexture tex) {
  ...
  if (!foundSpot) {
    newList = realloc(gRenderer->targets, (gRenderer->targetListSize +
                                           VK2D_DEFAULT_ARRAY_EXTENSION) *
                                              sizeof(VK2DTexture));
    ...
    if (newList != NULL) {
      ...
      for (i = gRenderer->targetListSize + 1;
           i < gRenderer->targetListSize + VK2D_DEFAULT_ARRAY_EXTENSION; i++)
        gRenderer->targets[i] = NULL;

      gRenderer->targetListSize += VK2D_DEFAULT_ARRAY_EXTENSION;
    } else {
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to extend targets list for %i target spots.",
                gRenderer->targetListSize);
    }
  }
}
...
void _vk2dTransitionImageLayout(VkImage img, VkImageLayout old,
  ...
  } else if (new == VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL &&
  ...
  vkCmdPipelineBarrier(gRenderer->commandBuffer[gRenderer->scImageIndex],
                       sourceStage, destinationStage, 0, 0, VK_NULL_HANDLE, 0,
...
void _vk2dCameraUpdateUBO(VK2DUniformBufferObject *ubo, VK2DCameraSpec *camera,
  ...
  if (camera->type == VK2D_CAMERA_TYPE_DEFAULT) {
    // Assemble view
    VK2DMat4 view = {0};
    VK2DVec3 eyes = {camera->x + (camera->w * 0.5),
                     camera->y + (camera->h * 0.5), -2};
    VK2DVec3 center = {eyes[0], eyes[1], 0};
    VK2DVec3 up = {sin(-camera->rot), -cos(-camera->rot), 0};
    cameraMatrix(view, eyes, center, up);

    // Projection
    VK2DMat4 proj = {0};
    orthographicMatrix(proj, camera->h / camera->zoom, camera->w / camera->h,
                       0.1, 10);
    ...
    // Assemble view
    VK2DMat4 view = {0};
    cameraMatrix(view, camera->Perspective.eyes, camera->Perspective.centre,
                 camera->Perspective.up);
    ...
    if (camera->type == VK2D_CAMERA_TYPE_ORTHOGRAPHIC)
      ...
      perspectiveMatrix(proj, camera->Perspective.fov, camera->w / camera->h,
                        0.1, 10);
...
//  Copies the camera ubos to the descriptor buffer and spits out the
...
void _vk2dRendererFlushUBOBuffers() {
  ...
  vk2dDescriptorBufferCopyData(
      ...
      &gRenderer->workingUBO, sizeof(VK2DUniformBufferObject), &buffer,
      &offset);
  VkDescriptorBufferInfo bufferInfo = {buffer, offset,
                                       sizeof(VK2DUniformBufferObject)};
  ...
  if (gRenderer->options.enableDebug && gRenderer != NULL) {
    fvkCreateDebugReportCallbackEXT =
        (PFN_vkCreateDebugReportCallbackEXT)vkGetInstanceProcAddr(
            gRenderer->vk, "vkCreateDebugReportCallbackEXT");
    fvkDestroyDebugReportCallbackEXT =
        (PFN_vkDestroyDebugReportCallbackEXT)vkGetInstanceProcAddr(
            gRenderer->vk, "vkDestroyDebugReportCallbackEXT");
    ...
    if (fvkCreateDebugReportCallbackEXT != NULL &&
      ...
      VkDebugReportCallbackCreateInfoEXT callbackCreateInfoEXT =
          vk2dInitDebugReportCallbackCreateInfoEXT(_vk2dDebugCallback);
      fvkCreateDebugReportCallbackEXT(gRenderer->vk, &callbackCreateInfoEXT,
                                      VK_NULL_HANDLE, &gRenderer->dr);
    } else {
...
void _vk2dRendererDestroyDebug() {
  ...
  if (gRenderer->options.enableDebug && gRenderer->dr != NULL) {
    fvkDestroyDebugReportCallbackEXT(gRenderer->vk, gRenderer->dr,
                                     VK_NULL_HANDLE);
  }
}
...
void _vk2dRendererGetSurfaceSize() {
  ...
  VkResult result = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      gRenderer->pd->dev, gRenderer->surface, &gRenderer->surfaceCapabilities);
  if (result != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to get surface size, Vulkan error %i.", result);
  } else {
    if (gRenderer->surfaceCapabilities.currentExtent.width == UINT32_MAX ||
      ...
      SDL_GetWindowSizeInPixels(gRenderer->window,
                                (void *)&gRenderer->surfaceWidth,
                                (void *)&gRenderer->surfaceHeight);
    } else {
      gRenderer->surfaceWidth =
          gRenderer->surfaceCapabilities.currentExtent.width;
      gRenderer->surfaceHeight =
          gRenderer->surfaceCapabilities.currentExtent.height;
    }
  }
}
...
void _vk2dRendererCreateWindowSurface() {
  ...
  if (gRenderer != NULL) {
    ...
    if (SDL_Vulkan_CreateSurface(gRenderer->window, gRenderer->vk,
                                 VK_NULL_HANDLE, &gRenderer->surface)) {
      VkResult result = vkGetPhysicalDeviceSurfacePresentModesKHR(
          gRenderer->pd->dev, gRenderer->surface, &gRenderer->presentModeCount,
          VK_NULL_HANDLE);
      if (result != VK_SUCCESS) {
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to get present modes, Vulkan error %i.", result);
        return;
      }
      gRenderer->presentModes =
          malloc(sizeof(VkPresentModeKHR) * gRenderer->presentModeCount);

      if (gRenderer->presentModes) {
        result = vkGetPhysicalDeviceSurfacePresentModesKHR(
            gRenderer->pd->dev, gRenderer->surface,
            &gRenderer->presentModeCount, gRenderer->presentModes);
        VkResult result2 = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
            gRenderer->pd->dev, gRenderer->surface,
            &gRenderer->surfaceCapabilities);
        if (result != VK_SUCCESS || result2 != VK_SUCCESS) {
          vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                    "Failed to get present modes, Vulkan error %i/%i.", result,
                    result2);
          return;
        }

        // You may want to search for a different format, but according to the
        // Vulkan hardware database, 100% of systems support
        ...
        vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                  "Failed to allocate %i present modes.",
                  gRenderer->presentModeCount);
      }
    } else {
      const char *error = SDL_GetError();
      if (error != NULL)
        vk2dLoggerLogf(VK2D_LOG_SEVERITY_ERROR, "SDL error: %s\n", error);
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR | VK2D_STATUS_SDL_ERROR,
                "Failed to create surface, SDL error: %s.", SDL_GetError());
    }
  }
}
...
void _vk2dRendererCreateSwapchain() {
  ...
  uint32_t imageCount = gRenderer->surfaceCapabilities.minImageCount > 3
                            ? gRenderer->surfaceCapabilities.minImageCount
                            : 3;
  if (gRenderer->surfaceCapabilities.maxImageCount > 0 &&
      imageCount > gRenderer->surfaceCapabilities.maxImageCount) {
    imageCount = gRenderer->surfaceCapabilities.maxImageCount;
  }

  gRenderer->config.screenMode = (VK2DScreenMode)_vk2dRendererGetPresentMode(
      (VkPresentModeKHR)gRenderer->config.screenMode);
  VkSwapchainCreateInfoKHR swapchainCreateInfoKHR =
      vk2dInitSwapchainCreateInfoKHR(
          gRenderer->surface, gRenderer->surfaceCapabilities,
          gRenderer->surfaceFormat, gRenderer->surfaceWidth,
          gRenderer->surfaceHeight,
          (VkPresentModeKHR)gRenderer->config.screenMode, VK_NULL_HANDLE,
          imageCount);
  VkBool32 supported;
  vkGetPhysicalDeviceSurfaceSupportKHR(
      gRenderer->pd->dev, gRenderer->pd->QueueFamily.graphicsFamily,
      gRenderer->surface, &supported);
  if (supported) {
    vkCreateSwapchainKHR(gRenderer->ld->dev, &swapchainCreateInfoKHR,
                         VK_NULL_HANDLE, &gRenderer->swapchain);

    VkResult result = vkGetSwapchainImagesKHR(
        gRenderer->ld->dev, gRenderer->swapchain,
        &gRenderer->swapchainImageCount, VK_NULL_HANDLE);
    if (result >= 0) {
      gRenderer->swapchainImageViews =
          calloc(1, gRenderer->swapchainImageCount * sizeof(VkImageView));
      gRenderer->swapchainImages =
          calloc(1, gRenderer->swapchainImageCount * sizeof(VkImage));
      if (gRenderer->swapchainImageViews && gRenderer->swapchainImages) {
        result = vkGetSwapchainImagesKHR(
            gRenderer->ld->dev, gRenderer->swapchain,
            &gRenderer->swapchainImageCount, gRenderer->swapchainImages);

        if (result >= 0) {
          for (i = 0; i < gRenderer->swapchainImageCount; i++) {
            VkImageViewCreateInfo imageViewCreateInfo =
                vk2dInitImageViewCreateInfo(gRenderer->swapchainImages[i],
                                            gRenderer->surfaceFormat.format,
                                            VK_IMAGE_ASPECT_COLOR_BIT, 1);
            result = vkCreateImageView(gRenderer->ld->dev, &imageViewCreateInfo,
                                       VK_NULL_HANDLE,
                                       &gRenderer->swapchainImageViews[i]);
            if (result < 0) {
              vk2dRaise(
                  VK2D_STATUS_VULKAN_ERROR,
                  "Failed to acquire swapchain image [%i], Vulkan error %i.", i,
                  result);
              break;
            }
          }
          vk2dLogInfo("Swapchain (%i images) initialized...",
                      swapchainCreateInfoKHR.minImageCount);
        } else {
          vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                    "Failed to acquire swapchain images, Vulkan error %i.",
                    result);
        }
      } else {
        vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                  "Failed to allocate swapchain image arrays.");
      ...
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to get swapchain images, Vulkan error %i.", result);
    }
  } else {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Physical device does not support KHR swapchain.");
  }
}
...
void _vk2dRendererDestroySwapchain() {
  ...
  uint32_t i;
  for (i = 0; i < gRenderer->swapchainImageCount; i++)
    vkDestroyImageView(gRenderer->ld->dev, gRenderer->swapchainImageViews[i],
                       VK_NULL_HANDLE);

  vkDestroySwapchainKHR(gRenderer->ld->dev, gRenderer->swapchain,
                        VK_NULL_HANDLE);
  free(gRenderer->swapchainImageViews);
  free(gRenderer->swapchainImages);
}
...
void _vk2dRendererCreateDepthBuffer() {
  ...
  VkFormat formats[] = {VK_FORMAT_D16_UNORM, VK_FORMAT_D32_SFLOAT,
                        VK_FORMAT_D32_SFLOAT_S8_UINT,
                        VK_FORMAT_D24_UNORM_S8_UINT};
  int formatCount = 4;
  VkFormat selectedFormat = VK_FORMAT_MAX_ENUM;

  for (int i = 0; i < formatCount && selectedFormat == VK_FORMAT_MAX_ENUM;
       i++) {
  ...
  if (selectedFormat != VK_FORMAT_MAX_ENUM) {
    ...
    gRenderer->depthBuffer = vk2dImageCreate(
        gRenderer->ld, gRenderer->surfaceWidth, gRenderer->surfaceHeight,
        gRenderer->depthBufferFormat, VK_IMAGE_ASPECT_DEPTH_BIT,
...
void _vk2dRendererCreateColourResources() {
  ...
  if (gRenderer->config.msaa != VK2D_MSAA_1X) {
    gRenderer->msaaImage = vk2dImageCreate(
        gRenderer->ld, gRenderer->surfaceWidth, gRenderer->surfaceHeight,
        gRenderer->surfaceFormat.format, VK_IMAGE_ASPECT_COLOR_BIT,
...
void _vk2dRendererCreateDescriptorBuffers() {
  ...
  if (vk2dStatusFatal())
    return;
  gRenderer->descriptorBuffers =
      malloc(sizeof(VK2DDescriptorBuffer) * VK2D_MAX_FRAMES_IN_FLIGHT);
  if (gRenderer->descriptorBuffers == NULL) {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate descriptor buffers array.");
    return;
  }
  for (int i = 0; i < VK2D_MAX_FRAMES_IN_FLIGHT; i++) {
    gRenderer->descriptorBuffers[i] =
        vk2dDescriptorBufferCreate(gRenderer->options.vramPageSize);
  }

  // Calculate max instances
  const int maxDrawInstances =
      gRenderer->options.vramPageSize / sizeof(VK2DDrawInstance);
  const int maxDrawCommands =
      gRenderer->options.vramPageSize / sizeof(VK2DDrawCommand);

  gRenderer->limits.maxInstancedDraws =
      maxDrawCommands < maxDrawInstances ? maxDrawCommands : maxDrawInstances;
  gRenderer->limits.maxInstancedDraws--;
...
void _vk2dRendererCreateRenderPass() {
  ...
  attachments[0].finalLayout = gRenderer->config.msaa > 1
  ...
  for (i = 0; i < colourAttachCount; i++) {
    ...
    subpassColourAttachments0[i].layout =
        VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  }
  ...
  subpassDepthAttachmentReference.layout =
  ...
  subpasses[0].pResolveAttachments =
      gRenderer->config.msaa > 1 ? &resolveAttachment : VK_NULL_HANDLE;
  ...
  VkRenderPassCreateInfo renderPassCreateInfo = vk2dInitRenderPassCreateInfo(
      attachments, attachCount, subpasses, subpassCount, &subpassDependency, 1);
  VkResult result =
      vkCreateRenderPass(gRenderer->ld->dev, &renderPassCreateInfo,
                         VK_NULL_HANDLE, &gRenderer->renderPass);
  if (result == VK_ERROR_OUT_OF_HOST_MEMORY) {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to create render pass, out of memory.");
    return;
  } else if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
    vk2dRaise(VK2D_STATUS_OUT_OF_VRAM,
              "Failed to create render pass, out of video memory.");
    return;
  }

  // Two more render passes for mid-frame render pass reset to swapchain and
  ...
  result =
      vkCreateRenderPass(gRenderer->ld->dev, &renderPassCreateInfo,
                         VK_NULL_HANDLE, &gRenderer->midFrameSwapRenderPass);
  if (result == VK_ERROR_OUT_OF_HOST_MEMORY) {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to create render pass, out of memory.");
    return;
  ...
  result =
      vkCreateRenderPass(gRenderer->ld->dev, &renderPassCreateInfo,
                         VK_NULL_HANDLE, &gRenderer->externalTargetRenderPass);
  if (result == VK_ERROR_OUT_OF_HOST_MEMORY) {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to create render pass, out of memory.");
    return;
...
void _vk2dRendererDestroyRenderPass() {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  vkDestroyRenderPass(gRenderer->ld->dev, gRenderer->renderPass,
                      VK_NULL_HANDLE);
  vkDestroyRenderPass(gRenderer->ld->dev, gRenderer->externalTargetRenderPass,
                      VK_NULL_HANDLE);
  vkDestroyRenderPass(gRenderer->ld->dev, gRenderer->midFrameSwapRenderPass,
                      VK_NULL_HANDLE);
}
...
void _vk2dRendererCreateDescriptorSetLayouts() {
  ...
  descriptorSetLayoutBinding[0] = vk2dInitDescriptorSetLayoutBinding(
      1, VK_DESCRIPTOR_TYPE_SAMPLER, 1, VK_SHADER_STAGE_FRAGMENT_BIT,
      VK_NULL_HANDLE);
  VkDescriptorSetLayoutCreateInfo descriptorSetLayoutCreateInfo =
      vk2dInitDescriptorSetLayoutCreateInfo(descriptorSetLayoutBinding,
                                            layoutCount);
  r1 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &descriptorSetLayoutCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslSampler);
  ...
  descriptorSetLayoutBindingShapes[0] = vk2dInitDescriptorSetLayoutBinding(
      0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT,
      VK_NULL_HANDLE);
  VkDescriptorSetLayoutCreateInfo shapesDescriptorSetLayoutCreateInfo =
      vk2dInitDescriptorSetLayoutCreateInfo(descriptorSetLayoutBindingShapes,
                                            shapeLayoutCount);
  shapesDescriptorSetLayoutCreateInfo.flags =
  ...
  r2 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &shapesDescriptorSetLayoutCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslBufferVP);

  // For user-created shaders
  const uint32_t userLayoutCount = 1;
  VkDescriptorSetLayoutBinding descriptorSetLayoutBindingUser[1];
  descriptorSetLayoutBindingUser[0] = vk2dInitDescriptorSetLayoutBinding(
      3, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
      VK_NULL_HANDLE);
  VkDescriptorSetLayoutCreateInfo userDescriptorSetLayoutCreateInfo =
      vk2dInitDescriptorSetLayoutCreateInfo(descriptorSetLayoutBindingUser,
                                            userLayoutCount);
  r3 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &userDescriptorSetLayoutCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslBufferUser);

  // For sampled textures
  const uint32_t texLayoutCount = 1;
  VkDescriptorSetLayoutBinding descriptorSetLayoutBindingTex[1];
  descriptorSetLayoutBindingTex[0] = vk2dInitDescriptorSetLayoutBinding(
  ...
  VkDescriptorSetLayoutCreateInfo texDescriptorSetLayoutCreateInfo =
      vk2dInitDescriptorSetLayoutCreateInfo(descriptorSetLayoutBindingTex,
                                            texLayoutCount);
  r4 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &texDescriptorSetLayoutCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslTexture);
  ...
  VkDescriptorSetLayoutCreateInfo texArraySetLayoutCreateInfo = {
      ...
      .bindingCount = 1};
  r5 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &texArraySetLayoutCreateInfo, VK_NULL_HANDLE,
                                   &gRenderer->dslTextureArray);
  ...
  VkDescriptorSetLayoutCreateInfo dslComputeCreateInfo = {
      ...
      .bindingCount = 2};
  r6 = vkCreateDescriptorSetLayout(gRenderer->ld->dev, &dslComputeCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslSpriteBatch);
  ...
  descriptorSetLayoutBindingSBO[0] = vk2dInitDescriptorSetLayoutBinding(
      3, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, VK_SHADER_STAGE_VERTEX_BIT,
      VK_NULL_HANDLE);
  VkDescriptorSetLayoutCreateInfo sboDescriptorSetLayoutCreateInfo =
      vk2dInitDescriptorSetLayoutCreateInfo(descriptorSetLayoutBindingSBO,
                                            sboLayoutCount);
  r7 = vkCreateDescriptorSetLayout(gRenderer->ld->dev,
                                   &sboDescriptorSetLayoutCreateInfo,
                                   VK_NULL_HANDLE, &gRenderer->dslBufferSBO);

  if (r1 != VK_SUCCESS || r2 != VK_SUCCESS || r3 != VK_SUCCESS ||
      r4 != VK_SUCCESS || r5 != VK_SUCCESS || r6 != VK_SUCCESS ||
      r7 != VK_SUCCESS) {
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create descriptor set layouts %i/%i/%i/%i/%i/%i/%i.",
...
void _vk2dRendererDestroyDescriptorSetLayout() {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslSampler,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslBufferVP,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslBufferUser,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslTexture,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslTextureArray,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslSpriteBatch,
                               VK_NULL_HANDLE);
  vkDestroyDescriptorSetLayout(gRenderer->ld->dev, gRenderer->dslBufferSBO,
                               VK_NULL_HANDLE);
}
...
void _vk2dRendererCreatePipelines() {
  ...
  if (vk2dStatusFatal())
    return;
  uint32_t i;
  VkPipelineVertexInputStateCreateInfo textureVertexInfo =
      _vk2dGetTextureVertexInputState();
  VkPipelineVertexInputStateCreateInfo colourVertexInfo =
      _vk2dGetColourVertexInputState();
  VkPipelineVertexInputStateCreateInfo modelVertexInfo =
      _vk2dGetModelVertexInputState();

  VkPipelineVertexInputStateCreateInfo instanceVertexInfo =
      _vk2dGetInstanceVertexInputState();
  VkPipelineVertexInputStateCreateInfo shadowsVertexInfo =
      _vk2dGetShadowsVertexInputState();
  ...
  // Texture pipeline
  VkDescriptorSetLayout instancedLayout[] = {
      gRenderer->dslBufferVP, gRenderer->dslSampler, gRenderer->dslTextureArray,
      gRenderer->dslBufferSBO};

  // Polygon pipelines
  gRenderer->primFillPipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderColourVert, shaderColourVertSize,
      shaderColourFrag, shaderColourFragSize, &gRenderer->dslBufferVP, 1,
      &colourVertexInfo, true, gRenderer->config.msaa,
      VK2D_PIPELINE_TYPE_DEFAULT);
  gRenderer->primLinePipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderColourVert, shaderColourVertSize,
      shaderColourFrag, shaderColourFragSize, &gRenderer->dslBufferVP, 1,
      &colourVertexInfo, false, gRenderer->config.msaa,
      VK2D_PIPELINE_TYPE_DEFAULT);

  // 3D pipelines
  gRenderer->modelPipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderModelVert, shaderModelVertSize,
      shaderModelFrag, shaderModelFragSize, instancedLayout, 3,
      &modelVertexInfo, true, gRenderer->config.msaa, VK2D_PIPELINE_TYPE_3D);
  gRenderer->wireframePipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderModelVert, shaderModelVertSize,
      shaderModelFrag, shaderModelFragSize, instancedLayout, 3,
      &modelVertexInfo, false, gRenderer->config.msaa, VK2D_PIPELINE_TYPE_3D);
  gRenderer->instancedPipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderInstancedVert, shaderInstancedVertSize,
      shaderInstancedFrag, shaderInstancedFragSize, instancedLayout, 4,
      &instanceVertexInfo, true, gRenderer->config.msaa,
      VK2D_PIPELINE_TYPE_INSTANCING);

  // Shadows pipeline
  gRenderer->shadowsPipe = vk2dPipelineCreate(
      gRenderer->ld, gRenderer->renderPass, gRenderer->surfaceWidth,
      gRenderer->surfaceHeight, shaderShadowsVert, shaderShadowsVertSize,
      shaderShadowsFrag, shaderShadowsFragSize, instancedLayout, 1,
      &shadowsVertexInfo, true, gRenderer->config.msaa,
      VK2D_PIPELINE_TYPE_SHADOWS);

  // Compute
  gRenderer->spriteBatchPipe = vk2dPipelineCreateCompute(
      gRenderer->ld, sizeof(VK2DComputePushBuffer), (void *)VK2DCompSpritebatch,
      sizeof(VK2DCompSpritebatch), &gRenderer->dslSpriteBatch, 1);
...
void _vk2dRendererCreateFrameBuffer() {
  ...
  if (vk2dStatusFatal())
    return;
  uint32_t i;
  gRenderer->framebuffers =
      malloc(sizeof(VkFramebuffer) * gRenderer->swapchainImageCount);
  ...
  if (gRenderer->framebuffers) {
    for (i = 0; i < gRenderer->swapchainImageCount; i++) {
      ...
      VkFramebufferCreateInfo framebufferCreateInfo =
          vk2dInitFramebufferCreateInfo(
              gRenderer->renderPass, gRenderer->surfaceWidth,
              gRenderer->surfaceHeight, attachments, attachCount);
      VkResult result =
          vkCreateFramebuffer(gRenderer->ld->dev, &framebufferCreateInfo,
                              VK_NULL_HANDLE, &gRenderer->framebuffers[i]);
      if (result == VK_SUCCESS) {
        vk2dLogInfo("Framebuffer[%i] ready...", i);
      } else {
        if (result == VK_ERROR_OUT_OF_HOST_MEMORY) {
          vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                    "Failed to create framebuffer, out of memory.");
        } else if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
          vk2dRaise(VK2D_STATUS_OUT_OF_VRAM,
                    "Failed to create framebuffer, out of video memory.");
        }
...
void _vk2dRendererDestroyFrameBuffer() {
  ...
  if (gRenderer->framebuffers != NULL) {
    for (i = 0; i < gRenderer->swapchainImageCount; i++)
      vkDestroyFramebuffer(gRenderer->ld->dev, gRenderer->framebuffers[i],
                           VK_NULL_HANDLE);
    free(gRenderer->framebuffers);
  }
}
...
void _vk2dRendererCreateUniformBuffers(bool newCamera) {
  ...
  if (newCamera) { // If the renderer has not yet been initialized
    VK2DCameraSpec cam = {VK2D_CAMERA_TYPE_DEFAULT,
                          0,
                          0,
                          gRenderer->surfaceWidth,
                          gRenderer->surfaceHeight,
  ...
  gRenderer->cameras[VK2D_DEFAULT_CAMERA].spec.wOnScreen =
      gRenderer->surfaceWidth;
  gRenderer->cameras[VK2D_DEFAULT_CAMERA].spec.hOnScreen =
      gRenderer->surfaceHeight;
...
void _vk2dRendererDestroyUniformBuffers() {
  ...
  for (i = 0; i < VK2D_MAX_CAMERAS; i++)
    if (vk2dCameraGetState(i) == VK2D_CAMERA_STATE_NORMAL ||
        vk2dCameraGetState(i) == VK2D_CAMERA_STATE_DISABLED)
      vk2dCameraSetState(i, VK2D_CAMERA_STATE_RESET);
}

void _vk2dRendererCreateSpriteBatching() {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  gRenderer->drawCommands = malloc(sizeof(struct VK2DDrawCommand) *
                                   gRenderer->limits.maxInstancedDraws);
  gRenderer->drawCommandCount = 0;

  if (gRenderer->drawCommands == NULL) {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate sprite batch of count %i.",
              gRenderer->limits.maxInstancedDraws);
  }
}
...
void _vk2dRendererCreateDescriptorPool(bool preserveDescCons) {
  ...
  if (vk2dStatusFatal())
    return;
  if (!preserveDescCons) {
    gRenderer->descConSamplers =
        vk2dDescConCreate(gRenderer->ld, gRenderer->dslTexture,
                          VK2D_NO_LOCATION, 2, VK2D_NO_LOCATION);
    gRenderer->descConSamplersOff =
        vk2dDescConCreate(gRenderer->ld, gRenderer->dslTexture,
                          VK2D_NO_LOCATION, 2, VK2D_NO_LOCATION);
    gRenderer->descConVP =
        vk2dDescConCreate(gRenderer->ld, gRenderer->dslBufferVP, 0,
                          VK2D_NO_LOCATION, VK2D_NO_LOCATION);
    vk2dDescConUpdateAfterBind(gRenderer->descConVP);
    gRenderer->descConUser =
        vk2dDescConCreate(gRenderer->ld, gRenderer->dslBufferUser, 3,
                          VK2D_NO_LOCATION, VK2D_NO_LOCATION);
    for (int i = 0; i < VK2D_MAX_FRAMES_IN_FLIGHT; i++) {
      gRenderer->descConCompute[i] =
          vk2dDescConCreate(gRenderer->ld, gRenderer->dslSpriteBatch,
                            VK2D_NO_LOCATION, VK2D_NO_LOCATION, 0);
      gRenderer->descConShaders[i] =
          vk2dDescConCreate(gRenderer->ld, gRenderer->dslBufferUser, 3,
                            VK2D_NO_LOCATION, VK2D_NO_LOCATION);
      gRenderer->descConSBO[i] =
          vk2dDescConCreate(gRenderer->ld, gRenderer->dslBufferSBO,
                            VK2D_NO_LOCATION, VK2D_NO_LOCATION, 3);
    }

    // And the one sampler set
    VkDescriptorPoolSize sizes = {VK_DESCRIPTOR_TYPE_SAMPLER, 4};
    VkDescriptorPoolCreateInfo descriptorPoolCreateInfo =
        vk2dInitDescriptorPoolCreateInfo(&sizes, 1, 4);
    VkResult result =
        vkCreateDescriptorPool(gRenderer->ld->dev, &descriptorPoolCreateInfo,
                               VK_NULL_HANDLE, &gRenderer->samplerPool);
    if (result == VK_SUCCESS) {
      VkDescriptorSetAllocateInfo descriptorSetAllocateInfo =
          vk2dInitDescriptorSetAllocateInfo(gRenderer->samplerPool, 1,
                                            &gRenderer->dslSampler);
      result = vkAllocateDescriptorSets(gRenderer->ld->dev,
                                        &descriptorSetAllocateInfo,
                                        &gRenderer->samplerSet);
      VkResult result2 = vkAllocateDescriptorSets(gRenderer->ld->dev,
                                                  &descriptorSetAllocateInfo,
                                                  &gRenderer->modelSamplerSet);

      if (result != VK_SUCCESS || result2 != VK_SUCCESS)
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to allocate descriptor set, Vulkan error %i/%i.",
                  result, result2);
      else
        vk2dLogInfo("Descriptor controllers initialized...");
    } else {
      vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                "Failed to allocate descriptor pool, Vulkan error %i.", result);
    }
    ...
    VkResult texArrayResult =
        vkCreateDescriptorPool(gRenderer->ld->dev, &texArrayPoolCreateInfo,
                               VK_NULL_HANDLE, &gRenderer->texArrayPool);
    if (texArrayResult != VK_SUCCESS) {
      vk2dRaise(
          VK2D_STATUS_VULKAN_ERROR,
          "Failed to allocate texture array descriptor pool, Vulkan error %i.",
          result);
    }
    ...
    VkDescriptorSetAllocateInfo descriptorSetAllocateInfo = {
        ...
        .pNext = &setCounts};
    result =
        vkAllocateDescriptorSets(gRenderer->ld->dev, &descriptorSetAllocateInfo,
                                 &gRenderer->texArrayDescriptorSet);
    if (result != VK_SUCCESS) {
      vk2dRaise(
          VK2D_STATUS_VULKAN_ERROR,
          "Failed to allocate texture array descriptor set, Vulkan error %i.",
          result);
    }

    // Make the descriptor tracker
    gRenderer->textureArray =
        calloc(gRenderer->options.maxTextures,
               sizeof(struct VK2DTextureDescriptorInfo_t));
    if (gRenderer->textureArray == NULL) {
      vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                "Failed to allocate texture info for %i potential textures.",
                gRenderer->options.maxTextures);
    }
...
void _vk2dRendererDestroyDescriptorPool(bool preserveDescCons) {
  ...
  if (!preserveDescCons) {
    ...
    vkDestroyDescriptorPool(gRenderer->ld->dev, gRenderer->samplerPool,
                            VK_NULL_HANDLE);
    vkDestroyDescriptorPool(gRenderer->ld->dev, gRenderer->texArrayPool,
                            VK_NULL_HANDLE);
...
void _vk2dRendererCreateSynchronization() {
  ...
  VkFenceCreateInfo fenceCreateInfo =
      vk2dInitFenceCreateInfo(VK_FENCE_CREATE_SIGNALED_BIT);
  gRenderer->imageAvailableSemaphores =
      calloc(1, sizeof(VkSemaphore) * VK2D_MAX_FRAMES_IN_FLIGHT);
  gRenderer->renderFinishedSemaphores =
      calloc(1, sizeof(VkSemaphore) * gRenderer->swapchainImageCount);
  gRenderer->inFlightFences =
      calloc(1, sizeof(VkFence) * VK2D_MAX_FRAMES_IN_FLIGHT);
  gRenderer->imagesInFlight =
      calloc(1, sizeof(VkFence) * gRenderer->swapchainImageCount);
  gRenderer->commandBuffer =
      calloc(1, sizeof(VkCommandBuffer) * gRenderer->swapchainImageCount);
  gRenderer->dbCommandBuffer =
      calloc(1, sizeof(VkCommandBuffer) * gRenderer->swapchainImageCount);
  gRenderer->computeCommandBuffer =
      calloc(1, sizeof(VkCommandBuffer) * gRenderer->swapchainImageCount);

  if (gRenderer->imageAvailableSemaphores != NULL &&
      gRenderer->renderFinishedSemaphores != NULL &&
      gRenderer->inFlightFences != NULL && gRenderer->imagesInFlight != NULL) {
    for (i = 0; i < VK2D_MAX_FRAMES_IN_FLIGHT; i++) {
      VkResult r3 =
          vkCreateFence(gRenderer->ld->dev, &fenceCreateInfo, VK_NULL_HANDLE,
                        &gRenderer->inFlightFences[i]);
      VkResult r1 = vkCreateSemaphore(gRenderer->ld->dev, &semaphoreCreateInfo,
                                      VK_NULL_HANDLE,
                                      &gRenderer->imageAvailableSemaphores[i]);
      if (r3 != VK_SUCCESS || r1 != VK_SUCCESS)
        vk2dRaise(
            VK2D_STATUS_VULKAN_ERROR,
            "Failed to create synchronization objects, Vulkan error %i/%i.", r1,
            r3);
    }
    for (i = 0; i < gRenderer->swapchainImageCount; i++) {
      VkResult r2 = vkCreateSemaphore(gRenderer->ld->dev, &semaphoreCreateInfo,
                                      VK_NULL_HANDLE,
                                      &gRenderer->renderFinishedSemaphores[i]);
      if (r2 != VK_SUCCESS)
        vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
                  "Failed to create synchronization objects, Vulkan error %i.",
                  r2);
    }
  } else {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
              "Failed to allocate synchronization objects.");
  }

  if (gRenderer->commandBuffer != NULL && gRenderer->dbCommandBuffer != NULL) {
    for (i = 0; i < gRenderer->swapchainImageCount; i++) {
      gRenderer->commandBuffer[i] =
          vk2dLogicalDeviceGetCommandBuffer(gRenderer->ld, true);
      gRenderer->dbCommandBuffer[i] =
          vk2dLogicalDeviceGetCommandBuffer(gRenderer->ld, true);
      gRenderer->computeCommandBuffer[i] =
          vk2dLogicalDeviceGetCommandBuffer(gRenderer->ld, true);
    }
  } else {
    vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
...
void _vk2dRendererDestroySynchronization() {
  ...
  if (gRenderer->renderFinishedSemaphores != NULL &&
      gRenderer->imageAvailableSemaphores != NULL &&
      gRenderer->inFlightFences != NULL) {
    for (i = 0; i < VK2D_MAX_FRAMES_IN_FLIGHT; i++) {
      vkDestroySemaphore(gRenderer->ld->dev,
                         gRenderer->imageAvailableSemaphores[i],
                         VK_NULL_HANDLE);
      vkDestroyFence(gRenderer->ld->dev, gRenderer->inFlightFences[i],
                     VK_NULL_HANDLE);
    }
...
void _vk2dRendererCreateSampler() {
  ...
  // 2D sampler
  VkSamplerCreateInfo samplerCreateInfo = vk2dInitSamplerCreateInfo(
      gRenderer->config.filterMode == VK2D_FILTER_TYPE_LINEAR,
      gRenderer->config.filterMode == VK2D_FILTER_TYPE_LINEAR
          ? gRenderer->config.msaa
          : 1,
      1);
  VkResult r1 = vkCreateSampler(gRenderer->ld->dev, &samplerCreateInfo,
                                VK_NULL_HANDLE, &gRenderer->textureSampler);
  VkDescriptorImageInfo imageInfo = {0};
  imageInfo.sampler = gRenderer->textureSampler;
  VkWriteDescriptorSet write = vk2dInitWriteDescriptorSet(
      VK_DESCRIPTOR_TYPE_SAMPLER, 1, gRenderer->samplerSet, VK_NULL_HANDLE, 1,
      &imageInfo);
  vkUpdateDescriptorSets(gRenderer->ld->dev, 1, &write, 0, VK_NULL_HANDLE);
  ...
  VkResult r2 = vkCreateSampler(gRenderer->ld->dev, &samplerCreateInfo,
                                VK_NULL_HANDLE, &gRenderer->modelSampler);
  imageInfo.sampler = gRenderer->modelSampler;
  write = vk2dInitWriteDescriptorSet(VK_DESCRIPTOR_TYPE_SAMPLER, 1,
                                     gRenderer->modelSamplerSet, VK_NULL_HANDLE,
                                     1, &imageInfo);
  vkUpdateDescriptorSets(gRenderer->ld->dev, 1, &write, 0, VK_NULL_HANDLE);
  ...
  if (r1 == VK_SUCCESS && r2 == VK_SUCCESS)
    ...
    vk2dRaise(VK2D_STATUS_VULKAN_ERROR,
              "Failed to create sampler descriptor sets, Vulkan error %i/%i.",
              r1, r2);
}

void _vk2dRendererDestroySampler() {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  vkDestroySampler(gRenderer->ld->dev, gRenderer->textureSampler,
                   VK_NULL_HANDLE);
...
void _vk2dRendererCreateUnits() {
  ...
  if (vk2dStatusFatal())
    return;
  // Squares are simple
  gRenderer->unitSquare =
      vk2dPolygonShapeCreateRaw(unitSquare, unitSquareVertices);
  gRenderer->unitSquareOutline =
      vk2dPolygonCreateOutline(unitSquareOutline, unitSquareOutlineVertices);

  // Now to generate the circle
  VK2DVec2 *circleVertices = malloc(sizeof(VK2DVec2) * VK2D_CIRCLE_VERTICES);
  uint32_t i;
  for (i = 0; i < VK2D_CIRCLE_VERTICES; i++) {
    circleVertices[i][0] =
        cos(((double)i / VK2D_CIRCLE_VERTICES) * (VK2D_PI * 2)) * 0.5;
    circleVertices[i][1] =
        sin(((double)i / VK2D_CIRCLE_VERTICES) * (VK2D_PI * 2)) * 0.5;
  }
  gRenderer->unitCircle =
      vk2dPolygonCreate(circleVertices, VK2D_CIRCLE_VERTICES);
  gRenderer->unitCircleOutline =
      vk2dPolygonCreateOutline(circleVertices, VK2D_CIRCLE_VERTICES);
  gRenderer->unitLine =
      vk2dPolygonCreateOutline((void *)LINE_VERTICES, LINE_VERTEX_COUNT);
  if (!vk2dStatusFatal())
...
void _vk2dImageTransitionImageLayout(VK2DLogicalDevice dev, VkImage image,
  ...
  for (i = 0; i < gRenderer->targetListSize; i++) {
    if (gRenderer->targets[i] != NULL) {
      ...
      gRenderer->targets[i]->sampledImg = vk2dImageCreate(
          gRenderer->ld, gRenderer->targets[i]->img->width,
          gRenderer->targets[i]->img->height, VK_FORMAT_B8G8R8A8_SRGB,
          VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
          (VkSampleCountFlagBits)gRenderer->config.msaa);
      _vk2dImageTransitionImageLayout(
          gRenderer->ld, gRenderer->targets[i]->sampledImg->img,
          VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
      //_vk2dImageTransitionImageLayout(gRenderer->ld,
      // gRenderer->targets[i]->depthBuffer->img, VK_IMAGE_LAYOUT_UNDEFINED,
      // VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
      gRenderer->targets[i]->depthBuffer = vk2dImageCreate(
          gRenderer->ld, gRenderer->targets[i]->img->width,
          gRenderer->targets[i]->img->height, gRenderer->depthBufferFormat,
          VK_IMAGE_ASPECT_DEPTH_BIT,
          VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
          (VkSampleCountFlagBits)gRenderer->config.msaa);

      // Framebuffer
      vkDestroyFramebuffer(gRenderer->ld->dev, gRenderer->targets[i]->fbo,
                           VK_NULL_HANDLE);
      const int attachCount = gRenderer->config.msaa > 1 ? 3 : 2;
      VkImageView attachments[3];
      if (gRenderer->config.msaa > 1) {
      ...
      VkFramebufferCreateInfo framebufferCreateInfo =
          vk2dInitFramebufferCreateInfo(gRenderer->externalTargetRenderPass,
                                        gRenderer->targets[i]->img->width,
                                        gRenderer->targets[i]->img->height,
                                        attachments, attachCount);
      VkResult result =
          vkCreateFramebuffer(gRenderer->ld->dev, &framebufferCreateInfo,
                              VK_NULL_HANDLE, &gRenderer->targets[i]->fbo);
      if (result == VK_ERROR_OUT_OF_HOST_MEMORY) {
        vk2dRaise(VK2D_STATUS_OUT_OF_RAM,
                  "Failed to create framebuffer, out of memory.");
      } else if (result == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
...
void nkFree(nk_handle handle, void *old) { free(old); }
...
void _vk2dRendererInitNuklear() {
  ...
  if (vk2d->options.enableNuklear) {
    ...
    struct nk_context *ctx = nk_sdl_init(
        vk2d->window, device, physical_device, graphics_queue_family_index,
        image_views, image_views_len, COLOR_FORMAT, INIT_STATE,
...
void _vk2dRendererResetNuklear() {
  ...
  if (gRenderer->options.enableNuklear) {
    ...
    nk_sdl_resize(gRenderer->swapchainImageViews,
                  gRenderer->swapchainImageCount, gRenderer->surfaceWidth,
                  gRenderer->surfaceHeight);
  }
}
...
void _vk2dRendererDrawRaw(VkDescriptorSet *sets, uint32_t setCount,
  ...
  // Only do rotation matrices if a rotation is specified for optimization
  // purposes
  if (rot != 0) {
    VK2DVec3 axis = {0, 0, 1};
    VK2DVec3 origin = {-originX + x, originY + y, 0};
    VK2DVec3 originTranslation = {originX, -originY, 0};
    translateMatrix(push.model, origin);
    rotateMatrix(push.model, axis, rot);
    translateMatrix(push.model, originTranslation);
  } else {
    VK2DVec3 origin = {x, y, 0};
    translateMatrix(push.model, origin);
  }
  // Only scale matrix if specified for optimization purposes
  if (xscale != 1 || yscale != 1) {
    VK2DVec3 scale = {xscale, yscale, 1};
  ...
  if (gRenderer->prevPipe != vk2dPipelineGetPipe(pipe, gRenderer->blendMode)) {
    vkCmdBindPipeline(buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      vk2dPipelineGetPipe(pipe, gRenderer->blendMode));
    gRenderer->prevPipe = vk2dPipelineGetPipe(pipe, gRenderer->blendMode);
  }
  if (gRenderer->prevSetHash != hash) {
    vkCmdBindDescriptorSets(buf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipe->layout,
                            0, setCount, sets, 0, VK_NULL_HANDLE);
  ...
  cam = cam == VK2D_INVALID_CAMERA ? VK2D_DEFAULT_CAMERA
                                   : cam; // Account for invalid camera
  VkRect2D scissor;
  VkViewport viewport;
  ...
  vkCmdPushConstants(buf, pipe->layout,
                     VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                     0, sizeof(VK2DPushBuffer), &push);
  if (poly != NULL)
    vkCmdDraw(buf, poly->vertexCount, 1, 0, 0);
  else // The only time this would be the case is for textures, where the shader
...
void _vk2dRendererDrawRawShader(VkDescriptorSet *sets, uint32_t setCount,
  ...
  vkCmdPushConstants(buf, pipe->layout,
                     VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                     0, sizeof(VK2DShaderPushBuffer), &push);
  vkCmdDraw(buf, 6, 1, 0, 0);
}
...
void _vk2dRendererDrawRawShadows(VkDescriptorSet set,
  ...
  push.cameraIndex = cam == VK2D_INVALID_CAMERA ? 0 : cam;
  memcpy(push.model, objInfo->model, sizeof(VK2DMat4));
  // Check if we actually need to bind things
  if (gRenderer->prevPipe != vk2dPipelineGetPipe(pipe, gRenderer->blendMode)) {
    vkCmdBindPipeline(buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      vk2dPipelineGetPipe(pipe, gRenderer->blendMode));
  ...
  vkCmdBindDescriptorSets(buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          gRenderer->shadowsPipe->layout, 0, 1, &set, 0,
                          VK_NULL_HANDLE);
  ...
  vkCmdPushConstants(buf, pipe->layout,
                     VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                     0, sizeof(VK2DShadowsPushBuffer), &push);
...
void _vk2dRendererDrawRawInstanced(VkDescriptorSet *sets, uint32_t setCount,
  ...
  vkCmdBindPipeline(
      ...
      vk2dPipelineGetPipe(gRenderer->instancedPipe, gRenderer->blendMode));
  vkCmdBindDescriptorSets(buf, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          gRenderer->instancedPipe->layout, 0, setCount, sets,
                          0, VK_NULL_HANDLE);
  ...
  VkDeviceSize offset;
  vk2dDescriptorBufferCopyData(
      gRenderer->descriptorBuffers[gRenderer->currentFrame], instances,
      count * sizeof(VK2DDrawInstance), &buffer, &offset);
...
void _vk2dRendererDrawRaw3D(VkDescriptorSet *sets, uint32_t setCount,
  ...
  identityMatrix(push.model);
  VK2DVec3 originTranslation = {-originX, -originY, -originZ};
  VK2DVec3 origin2 = {originX + x, originY + y, originZ + z};
  VK2DVec3 scale = {xscale, yscale, zscale};
  ...
  vkCmdBindIndexBuffer(buf, model->vertices->buf, model->indexOffset,
  ...
  vkCmdPushConstants(buf, pipe->layout,
                     VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                     0, sizeof(VK2D3DPushBuffer), &push);
...
void _vk2dRendererDraw3D(VkDescriptorSet *sets, uint32_t setCount,
  ...
  for (int i = 0; i < VK2D_MAX_CAMERAS; i++) {
    if (gRenderer->cameras[i].state == VK2D_CAMERA_STATE_NORMAL &&
        gRenderer->cameras[i].spec.type != VK2D_CAMERA_TYPE_DEFAULT &&
        (i == gRenderer->cameraLocked ||
         gRenderer->cameraLocked == VK2D_INVALID_CAMERA)) {
      sets[0] = gRenderer->uboDescriptorSets[gRenderer->currentFrame];
      _vk2dRendererDrawRaw3D(sets, setCount, model, pipe, x, y, z, xscale,
                             yscale, zscale, rot, axis, originX, originY,
                             originZ, i, lineWidth);
    }
  }
}

// This is the upper level internal draw function that draws to each camera and
// not just with a scissor/viewport
void _vk2dRendererDraw(VkDescriptorSet *sets, uint32_t setCount,
  ...
  if (gRenderer->target != VK2D_TARGET_SCREEN &&
      !gRenderer->enableTextureCameraUBO) {
    sets[0] = gRenderer->targetUBOSet;
    _vk2dRendererDrawRaw(sets, setCount, poly, pipe, x, y, xscale, yscale, rot,
                         originX, originY, lineWidth, xInTex, yInTex, texWidth,
                         texHeight, 0);
  } else {
    ...
    for (int i = 0; i < VK2D_MAX_CAMERAS; i++) {
      if (gRenderer->cameras[i].state == VK2D_CAMERA_STATE_NORMAL &&
          ...
          (i == gRenderer->cameraLocked ||
           gRenderer->cameraLocked == VK2D_INVALID_CAMERA)) {
        sets[0] = gRenderer->uboDescriptorSets[gRenderer->currentFrame];
        _vk2dRendererDrawRaw(sets, setCount, poly, pipe, x, y, xscale, yscale,
                             rot, originX, originY, lineWidth, xInTex, yInTex,
                             texWidth, texHeight, i);
      }
    }
  }
}
...
void _vk2dRendererDrawShader(VkDescriptorSet *sets, uint32_t setCount,
  ...
  if (gRenderer->target != VK2D_TARGET_SCREEN &&
      !gRenderer->enableTextureCameraUBO) {
    sets[0] = gRenderer->targetUBOSet;
    _vk2dRendererDrawRawShader(sets, setCount, tex, pipe, x, y, xscale, yscale,
                               rot, originX, originY, lineWidth, xInTex, yInTex,
                               texWidth, texHeight, VK2D_INVALID_CAMERA);
  } else {
    // Only render to 2D cameras
    for (int i = 0; i < VK2D_MAX_CAMERAS; i++) {
      if (gRenderer->cameras[i].state == VK2D_CAMERA_STATE_NORMAL &&
          ...
          (i == gRenderer->cameraLocked ||
           gRenderer->cameraLocked == VK2D_INVALID_CAMERA)) {
        sets[0] = gRenderer->uboDescriptorSets[gRenderer->currentFrame];
        _vk2dRendererDrawRawShader(sets, setCount, tex, pipe, x, y, xscale,
                                   yscale, rot, originX, originY, lineWidth,
                                   xInTex, yInTex, texWidth, texHeight, i);
      }
    }
  }
}

// This is the upper level internal draw function for shadows that draws to each
// camera and not just with a scissor/viewport
  ...
  if (gRenderer->target != VK2D_TARGET_SCREEN &&
    ...
    for (int so = 0; so < shadowEnvironment->objectCount; so++) {
      if (shadowEnvironment->objectInfos[so].enabled)
        _vk2dRendererDrawRawShadows(set, shadowEnvironment, so, colour,
                                    lightSource, VK2D_INVALID_CAMERA);
    }
  } else {
    // Only render to 2D cameras
    for (int i = 0; i < VK2D_MAX_CAMERAS; i++) {
...
static void _vk2dRendererAddDrawCommandInternal(VK2DDrawCommand *command) {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  memcpy(&gRenderer->drawCommands[gRenderer->drawCommandCount++], command,
         sizeof(struct VK2DDrawCommand));
}
...
void _vk2dRendererFlushBatchIfNeeded(VK2DPipeline pipe) {
  VK2DRenderer gRenderer = vk2dRendererGetPointer();
  if (vk2dPipelineGetID(pipe, gRenderer->blendMode) !=
          gRenderer->currentBatchPipelineID ||
      gRenderer->drawCommandCount >= gRenderer->limits.maxInstancedDraws) {
