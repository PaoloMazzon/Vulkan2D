...
/// this is here instead of constants to avoid circular dependencies
...
/// \brief Blend modes that can be used to render if VK2D_GENERATE_BLEND_MODES
/// is enabled
///
/// The blend modes are kind of limited because very few people need more than
/// these, but if you require a different blend mode there is only 3 steps to
/// adding it:
///
...
typedef enum {
  ...
  VK2D_BLEND_MODE_SUBTRACT =
      3, ///< Subtraction blending, new colour is subtracted from current colour
...
/// you request a mode that is not available, the option will default to
/// sm_Vsync.
typedef enum {
  VK2D_SCREEN_MODE_IMMEDIATE =
      VK_PRESENT_MODE_IMMEDIATE_KHR, ///< Quickest mode, just plop to screen but
                                     ///< may have screen tearing
  VK2D_SCREEN_MODE_VSYNC =
      VK_PRESENT_MODE_FIFO_KHR, ///< Slower but prevents screen tearing
  VK2D_SCREEN_MODE_TRIPLE_BUFFER =
      VK_PRESENT_MODE_MAILBOX_KHR ///< Optimal for gaming but a bit slower than
                                  ///< immediate (machines may not support this)
} VK2DScreenMode;

/// \brief Specifies how textures will be filtered at higher and lower
/// resolutions
typedef enum {
  VK2D_FILTER_TYPE_LINEAR =
      VK_FILTER_LINEAR, ///< Linear interpolation, good for most things
  VK2D_FILTER_TYPE_NEAREST =
      VK_FILTER_NEAREST ///< Nearest neighbor filter, good for pixel art
} VK2DFilterType;

/// \brief A bitwise-able enum representing different shader stages
typedef enum {
  VK2D_SHADER_STAGE_FRAGMENT =
      VK_SHADER_STAGE_FRAGMENT_BIT, ///< Fragment (pixel) shader
...
typedef enum {
  ...
  VK2D_CAMERA_STATE_DELETED =
      2,                       ///< Camera is "deleted" and all data is invalid
...
typedef enum {
  ...
  VK2D_RESET_SWAPCHAIN = 1, ///< The swapchain (renderer) was just reset (likely
                            ///< due to window resize or something similar)
...
typedef enum {
  ...
  VK2D_STATUS_FILE_NOT_FOUND =
      1
      << 0, ///< File was not found for something like an image load, not fatal
  VK2D_STATUS_BAD_FORMAT = 1 << 1, ///< Bad file format
  VK2D_STATUS_TOO_MANY_CAMERAS =
      1 << 2, ///< No camera slots left to use, not fatal
  VK2D_STATUS_DEVICE_LOST =
      1 << 3, ///< General Vulkan catch-all for when something goes wrong
  ...
  VK2D_STATUS_RENDERER_NOT_INITIALIZED =
      1 << 7,                     ///< Renderer has not been initialized
  VK2D_STATUS_SDL_ERROR = 1 << 8, ///< General SDL-catch all, not fatal
  VK2D_STATUS_BEYOND_LIMIT =
      1
      << 9, ///< User requested a setting that was beyond host limits, not fatal
  VK2D_STATUS_BAD_ASSET =
      1 << 10, ///< User tried to pass a NULL asset to a VK2D method, not fatal
} VK2DStatus;
...
typedef float VK2DVec2[2];

/// \brief 3D vector of floats
typedef float VK2DVec3[3];

/// \brief 4D vector of floats
typedef float VK2DVec4[4];

/// \brief 4x4 matrix of floats
typedef float VK2DMat4[16];
...
struct VK2DVertexColour {
  VK2DVec3 pos;    ///< Position of this vertex
  VK2DVec4 colour; ///< Colour of this vertex
};

/// \brief Vertex data for 3D models
struct VK2DVertex3D {
  VK2DVec3 pos; ///< Position of the vertex
  VK2DVec2 uv;  ///< UV coordinates for this vertex
};

/// \brief The VP buffer
struct VK2DUniformBufferObject {
  VK2DMat4 viewproj[VK2D_MAX_CAMERAS]; ///< View and projection matrix
                                       ///< multiplied together
};
...
struct VK2DShaderPushBuffer {
  ...
  VK2DVec2 _padding;     ///< For VK2DVec4 alignment
  VK2DVec4 texturePos;   ///< UV coordinates (unnormalized)
  VK2DVec4 colour;       ///< Colour mod of the renderer when called
  VK2DMat4 model;        ///< Model matrix
};

/// \brief Buffer passed per-model via push constants
struct VK2DPushBuffer {
  VK2DMat4 model;      ///< Model matrix
  VK2DVec4 colourMod;  ///< Current colour modifier
...
struct VK2D3DPushBuffer {
  VK2DMat4 model;        ///< Model matrix
  VK2DVec4 colourMod;    ///< Color modifier
...
struct VK2DShadowsPushBuffer {
  VK2DMat4 model;       ///< Model matrix for this shadow object
  VK2DVec2 lightSource; ///< Light source position
  VK2DVec2 _alignment;  ///< Simply for memory alignment
  VK2DVec4 colour;      ///< Colour of this shadow render
  uint32_t cameraIndex; ///< Index of the camera
};
...
struct VK2DStartupOptions {
  ...
  const char *errorFile; ///< The file to output errors to, or NULL to disable
                         ///< file output
  ...
  /// Determines the size of a video-memory page in bytes. This can cap the max
  /// uniform buffer size for shaders, max instances in one instanced call, and
  /// max vertices in a single geometry render. You may leave this as 0, in
  /// which case the renderer will make it 256kb.
  uint64_t vramPageSize;
};

/// \brief User configurable settings
/// \warning Currently filterMode cannot be changed after the renderer is
/// created but this is likely going to be fixed later
struct VK2DRendererConfig {
  VK2DMSAA msaa;             ///< Current MSAA
  VK2DScreenMode screenMode; ///< Current screen mode
  VK2DFilterType filterMode; ///< How to filter textures -- Not change-able
...
struct VK2DCameraSpec {
  ...
  float x;    ///< X position of the camera (top left coordinates) (only used in
              ///< default camera type)
  float y;    ///< Y position of the camera (top left coordinates) (only used in
              ///< default camera type)
  float w;    ///< Virtual width of the screen
  float h;    ///< Virtual height of the screen
  float zoom; ///< Zoom percentage (Relative to the virtual width and height,
              ///< not actual)
  ...
  ///< Things used for perspective and orthographic cameras in place of the x/y
  ///< variables
  struct {
    VK2DVec3 eyes;   ///< Where the 3D camera is
    VK2DVec3 centre; ///< Where the 3D camera is looking
    VK2DVec3 up;     ///< Which direction is up for the 3D camera
...
struct VK2DRendererLimits {
  VK2DMSAA maxMSAA; ///< Maximum MSAA the host supports, if you request an msaa
                    ///< higher than this value, this value will be used instead
  bool supportsTripleBuffering; ///< Whether or not the host supports triple
                                ///< buffering, if you request triple buffering
                                ///< and this is false, vsync will be used
                                ///< instead
  bool supportsImmediate; ///< Whether or not the host supports immediate mode,
                          ///< if you request immediate mode and this is false,
                          ///< vsync will be used instead
  bool supportsWireframe; ///< Doesn't really mean anything 💀
  float maxLineWidth; ///< Maximum line width supported on the platform, if you
                      ///< specify a line width greater than this value, your
                      ///< requested line width will be clamped to this number
  uint64_t
      maxInstancedDraws; ///< Maximum amount of instances you may draw at once,
                         ///< if you request to draw more instances than this it
                         ///< will simply be capped to this number
  uint64_t maxShaderBufferSize; ///< Maximum size of a shader's uniform buffer
                                ///< in bytes, if you attempt to create a shader
                                ///< with a uniform buffer size greater than
                                ///< this value NULL will be returned
  uint64_t
      maxGeometryVertices; ///< Maximum vertices that can be used in one
                           ///< vk2dRendererDrawGeometryCall, if you use more
                           ///< vertices than this nothing will happen.
  bool supportsMultiThreadLoading; ///< Whether or not the host supports loading
                                   ///< assets in another thread, if attempt to
                                   ///< load assets in another thread and this
                                   ///< is false, assets will be loaded on the
                                   ///< main thread instead
  bool supportsVRAMUsage; ///< Whether or not the host supports accurate VRAM
                          ///< usage, if this is false VMA will provide a less
                          ///< accurate estimate
};

/// \brief Represents the data you need for each element in an instanced draw
struct VK2DDrawInstance {
  VK2DVec4 texturePos;   ///< x in tex, y in tex, w in tex, and h in tex
  VK2DVec4 colour;       ///< Colour mod of this draw
  uint32_t textureIndex; ///< Which texture this instance is using
  VK2DVec3 padding;      ///< Padding
  VK2DMat4 model; ///< Model for this instance, generally shouldn't contain
                  ///< translations
};

/// \brief Represents a user's draw command which will later be processed into
/// an instance
struct VK2DDrawCommand {
  VK2DVec4 texturePos;   ///< x in tex, y in tex, w in tex, and h in tex
  VK2DVec4 colour;       ///< Colour mod of this draw
  VK2DVec2 pos;          ///< X/Y in game world for this instance
  VK2DVec2 origin;       ///< X/Y Origin of this draw
  VK2DVec2 scale;        ///< X/Y Scale of this draw
...
struct VK2DShadowObjectInfo {
  ...
  int startingVertex; ///< Vertex in the master VBO to start from for this
                      ///< object
  int vertexCount;    ///< Number of vertices for this model
  VK2DMat4 model;     ///< Model for this shadow object
};
...
struct VK2DAssetLoad {
  ...
  union {
    struct {
      const char
          *filename; ///< Filename to pull from or filename of the vertex shader
  ...
  union {
    ...
    struct {
      VK2DTexture *tex; ///< Texture to use for this model (pointer so the
                        ///< model's texture may be in the same list)
...
typedef void (*VK2DLoggerLogFn)(void *context, VK2DLogSeverity severity,
                                const char *message);
